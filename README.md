...
S3-совместимое хранилище для аватарок. REST для мутаций, GraphQL (Apollo, code-first) для чтения,
...
`redis` → `localhost`, `S3_PUBLIC_URL=http://localhost:9000`, `VITE_API_URL=http://localhost:3000`.
...
в nginx фронтенда, который раздаёт статику и проксирует `/api` в бэкенд, а `/s3` —
в хранилище. Один домен на всё, поэтому CORS не нужен.
...
- Между выдачей presigned-ссылки и подтверждением ключа возможен осиротевший объект
  в хранилище, если пользователь прервал загрузку. Сборка мусора не реализована.
- Содержимое картинки не проверяется по сигнатуре — проверяются тип и размер объекта.
